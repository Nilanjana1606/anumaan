...
prep_compute_poly_weights <- function(data,
  ...
  if (method == "monomicrobial_proportion") {
    ...
    message(sprintf(
      "Calculated monomicrobial proportions for %d organism(s).",
    ...
    if (n_fallback > 0) {
      message(sprintf(
        "Warning: %d isolate(s) used equal weighting (no monomicrobial reference).",
        n_fallback
      ))
    }
...
prep_split_poly_episode <- function(data,
  ...
  if (!polymicrobial_col %in% names(data)) {
    warning(sprintf(
      "[prep_split_poly_episode] Column '%s' not found. Returning data unchanged.",
      polymicrobial_col
    ))
    return(data)
  }
  ...
  message(sprintf(
    "[prep_split_poly_episode] Strategy: '%s'. Polymicrobial rows: %d / %d (%.1f%%).",
  ...
  if (strategy == "exclude") {
    result <- data[!is_poly, ]
    message(sprintf(
      "[prep_split_poly_episode] Excluded %d polymicrobial rows; %d rows retained.",
  ...
  if (strategy == "separate") {
    ...
    message(sprintf(
      "[prep_split_poly_episode] Main: %d rows; poly_data: %d rows.",
