...
prep_standardize_specimens <- function(data,
  ...
  collapse_sterile_classification <- function(specimen_name, sterile_classification) {
    ...
    ambiguous_specimens <- c(
      "fluid", "instrument", "others", "pigtail fluid",
      "superficial biopsy"
    )
    ...
    out[is.na(out) & class_key %in% c(
      "non-sterile", "non sterile",
      "non-sterile site", "non sterile site"
    )] <- "Non-Sterile"
    out[is.na(out) & class_key %in% c(
      "body fluid", "device-associated",
      "device associated", "others", "other",
      "others/ambiguous", "ambiguous"
    )] <- "Others/Ambiguous"
    out
  }
  ...
  for (input_spec in unique_inputs) {
    ...
    if (max_score > 0.3) {
      ...
      distances <- adist(input_spec, specimen_ref$temp_ref_lower, ignore.case = TRUE)[1, ]
