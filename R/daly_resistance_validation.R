...
#'   \item{Correlated}{\eqn{P(Y_{d_1}=1, Y_{d_2}=1 \mid \theta) =
...
validate_pairwise_calibration <- function(
  ...
  for (r in seq_len(nrow(hp_pairs))) {
    ...
    for (pr in pairs) {
      c1 <- pr[1L]
      c2 <- pr[2L]
      d1 <- match(c1, class_cols)
      d2 <- match(c2, class_cols)
      v1 <- obs_ast_mat[sub_idx, d1]
      v2 <- obs_ast_mat[sub_idx, d2]
      ...
      v1c <- v1[cotested_local]
      v2c <- v2[cotested_local]
  ...
  if (is_correlated && !requireNamespace("pbivnorm", quietly = TRUE)) {
    stop("[validate_pairwise_calibration] Package 'pbivnorm' is required to compute the ",
      "correlated-residual joint probability Phi_2(mu_d1, mu_d2; rho).",
      call. = FALSE
    )
  }
...
validate_complete_profile_calibration <- function(
  ...
  for (r in seq_len(nrow(hp_pairs))) {
    ...
    eligible_info[[key]] <- list(
      h = h_nm, k = k_nm, classes = panel$classes, d_idx = d_idx,
...
#'
#' \strong{Prediction}: for \code{fitted_model$residual_structure
...
#'   correlated-residual fits' masked-cell prediction. Ignored for
...
mask_and_validate_ast <- function(
  ...
  masked_list <- lapply(cell_groups, function(cell_df) {
    ...
    n_res <- sum(cell_df$value == 1)
    n_sus <- sum(cell_df$value == 0)
    ...
    res_df <- cell_df[cell_df$value == 1, ]
    sus_df <- cell_df[cell_df$value == 0, ]
  ...
  if (nrow(masked_df) == 0L) {
    stop("[mask_and_validate_ast] No cells could be masked without breaching the eligibility floors -- ",
      "lower fraction_to_mask/min_*_after_mask or check data support.",
      call. = FALSE
    )
  }

  message(sprintf(
    "[mask_and_validate_ast] Masking %d observed cell(s) across %d hospital x pathogen x class cell(s).",
  ...
  if (isTRUE(refit)) {
    if (is.null(fixed_effects)) {
      stop("[mask_and_validate_ast] `fixed_effects` must be supplied when refit = TRUE ",
        "(not recoverable from fitted_model; dummy-coding of categoricals is ambiguous).",
        call. = FALSE
      )
    }
    ...
    refit_model <- fit_bayesian_multivariate_probit(
      ...
      profile_group_col  = fitted_model$profile_group_col %||% fitted_model$upper_re_col,
  ...
  if (is_correlated) {
    # Correlated residual: predicting a masked cell from marginal
...
compute_profile_validation_status <- function(
  ...
  if (!is.null(complete_profile_tbl) && nrow(complete_profile_tbl) > 0L && "status" %in% names(complete_profile_tbl)) {
    ...
    n_eval <- if (nrow(evald) == 0L || length(panel_cols) == 0L) {
      0L
    } else {
      nrow(unique(evald[, panel_cols, drop = FALSE]))
    }
