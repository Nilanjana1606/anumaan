...
prep_get_contaminant_list <- function(syndrome = NULL,
  ...
  contaminant_patterns <- lapply(contaminants, function(name) {
    ...
    if (length(parts) >= 1) {
      ...
      if (is_genus_level) {
        patterns <- c(patterns, genus, genus_abbrev_alone)
      } else {
        patterns <- c(patterns, genus_abbrev_alone)
      }
...
prep_flag_contaminants <- function(data,
  ...
  if (context == "icu_device" &&
    ...
    if (any(is_clabsi_candidate, na.rm = TRUE)) {
      ...
      message(sprintf(
        "[prep_flag_contaminants] %d CoNS row(s) classified as CLABSI (not contaminant).",
...
prep_filter_fungal <- function(data,
  ...
  if (n_fungal > 0) {
    message(sprintf(
      "[prep_filter_fungal] Removing %d fungal row(s) (%.1f%%).",
