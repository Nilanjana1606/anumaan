...
surveillance data and estimating disease burden using GBD methodology. It is
structured around three sequential pipelines that can be used together or
separately.
...
**Vignette:** [Preprocessing Workflow](preprocessing-workflow.html)

The preprocessing pipeline standardises raw hospital or surveillance data into
an analysis-ready format. It covers:
...
- Organism, specimen, antibiotic, and AST value cleaning, including the
  0.1.0.9021 specimen reference additions and collapsed sterile-site classes
...
`run_preprocess()` for automated runs or the modular `prep_*` functions for
staged, inspectable workflows.
...
See the full step-by-step walkthrough in the
...
**Vignette:** [Exploratory Data Analysis](eda-workflow.html)

The EDA pipeline visualises preprocessed data before it moves on to burden
estimation. It does not transform data -- it is the review step a
stewardship team runs on Pipeline 1's output to catch data-quality issues and
characterise the cohort. It covers:

- Organism and antibiotic-susceptibility overviews (top organisms, R/S
  patterns, resistance heatmaps)
- Outcome distributions, including outcome by organism/resistance status,
  by age bin, and by year
...
All functions start with `plot_`, plus the shared `eda_theme()` styling
helper -- deliberately distinct from the `prep_` (Pipeline 1) and `daly_`
(Pipeline 3) naming.
...
[Exploratory Data Analysis vignette](eda-workflow.html).

---

## Pipeline 3 — DALY Burden Estimation

**Vignette:** [DALY Burden Estimation](daly-pipeline.html)

The DALY pipeline takes analysis-ready preprocessed data and produces
resistance-profile probability distributions and GBD-style burden estimates.
It covers:
...
All estimation functions start with `daly_`. Profile-specific functions are
`compute_*`, `estimate_*`, `enumerate_*`, `build_*`, `validate_*`,
...
`fit_bayesian_multivariate_probit()` now accepts a `compute` argument so the
same statistical model can be run on either a standard CPU backend or an
OpenCL-enabled backend.
...
This backend choice affects compilation and sampling only. Priors, likelihood,
diagnostics, posterior predictive checks, resistance profiles, and downstream
DALY quantities remain unchanged.
...
| Enhanced EDA theme | `ggpubr` (plots fall back to `ggplot2::theme_bw()` when absent) |

---

## Data Format

All three pipelines expect **long format** input — one row per
isolate × antibiotic combination:
...
`DOB`, `Age`, `gender`, `final_outcome`, `infection_type`, `state`.

`prep_standardize_specimens()` now returns `sterile_classification` collapsed to
`Sterile`, `Non-Sterile`, or `Others/Ambiguous`. The 0.1.0.9021 reference also
adds stewardship specimen labels such as `Brain abscess`, `Instrument`,
...
Full function documentation is at the
[Reference page](../reference/index.html).
