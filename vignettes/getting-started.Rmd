...
sample_data <- data.frame(
  ...
  date_of_admission = as.Date(c("2024-01-10", "2024-01-10",
                                 "2024-01-18", "2024-02-01", "2024-02-01")),
  date_of_culture   = as.Date(c("2024-01-15", "2024-01-15",
                                 "2024-01-20", "2024-02-03", "2024-02-03")),
  date_of_final_outcome = as.Date(c("2024-01-25", "2024-01-25",
                                     "2024-01-28", "2024-02-15", "2024-02-15")),
  final_outcome    = c("Survived", "Survived", "Died",   "Survived", "Survived"),
  specimen_type    = c("Blood",    "Blood",    "Urine",  "Blood",    "Blood"),
  organism_name    = c("E. coli",  "E. coli",  "K. pneumoniae",
                        "S. aureus", "S. aureus"),
  antibiotic_name  = c("Ampicillin", "Meropenem", "Ciprofloxacin",
                        "Oxacillin",  "Vancomycin"),
  antibiotic_value = c("R", "S", "R", "R", "S"),
  DOB              = as.Date(c("1960-05-01", "1960-05-01",
                                "1975-08-15", "1990-03-20", "1990-03-20")),
...
yll_associated <- daly_calc_yll_associated(
  ...
  date_culture_col = "date_of_culture",
  specimen_col     = "specimen_type"
)

# Attributable YLL: deaths caused by resistance (RR-based)
yll_attributable <- daly_calc_yll_attributable(
  yll_patient_data = yll_associated$patient_data,
  paf_mort         = paf_mortality,
  pathogen_col     = "organism_name",
  patient_col      = "patient_id",
  age_bin_col      = "Age_bin",
  sex_col          = ".sex_norm",
...
```{r yld}
yld_base <- daly_calc_yld_baseline(
  incidence_data = incidence_data,
  P_Lk_prime_tbl = p_lk_prime,
  DW_sepsis      = 0.133,
  avg_los_years  = 7 / 365.25
)

fraction_assoc <- daly_calc_fraction_associated_yld(profiles_with_rr)
paf_los        <- daly_calc_paf_los(profiles_with_rr)

yld_associated   <- anumaan:::daly_calc_yld_associated(yld_base, fraction_assoc)
yld_attributable <- daly_calc_yld_attributable(yld_base, paf_los)

yld_summary <- dplyr::left_join(
  yld_associated[, c("pathogen", "YLD_associated")],
  yld_attributable[, c("pathogen", "YLD_attributable")],
  by = "pathogen"
)
```
...
hospital_daly <- compute_hospital_daly(
  ...
  total_deaths     = sum(centre_counts$deaths_h),
  total_discharged = sum(centre_counts$discharged_h),
  yll_base         = yll_associated$total,
  yll_associated   = yll_associated$total,
  yll_attributable = yll_attributable$total,
  yld_base         = sum(yld_base$YLD, na.rm = TRUE),
  yld_associated   = sum(yld_associated$YLD_associated, na.rm = TRUE),
  yld_attributable = sum(yld_attributable$YLD_attributable, na.rm = TRUE)
)
```
...
plot_burden_by_organism(
  data         = yll_associated$per_pathogen |>
    dplyr::left_join(yll_attributable$per_pathogen, by = "pathogen"),
  metric       = "YLL",
  organism_col = "pathogen"
)

plot_yll_heatmap(
  data         = yll_attributable$by_profile,
  type         = "attributable",
  value_col    = "YLL_attributable_k",
  n_admissions = nrow(result$data)
)
