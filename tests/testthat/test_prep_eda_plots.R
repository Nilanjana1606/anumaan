...
test_that("plot_syndrome_distribution returns ggplot for all modes and errors correctly", {
  ...
  expect_s3_class(plot_syndrome_distribution(make_amr_data(),
    mode = "single",
    center = "Centre A"
  ), "ggplot")
  expect_error(
    plot_syndrome_distribution(make_amr_data(), mode = "single"),
    "'center' must be provided"
  )
  bad <- make_amr_data()
  bad$infectious_syndrome <- NULL
...
test_that("plot_resistance_by_agebin returns ggplot for all modes and errors correctly", {
  d <- make_amr_data()
  expect_s3_class(plot_resistance_by_agebin(d,
    mode = "overall",
  ...
  expect_s3_class(plot_resistance_by_agebin(d,
    mode = "single", center = "Centre A",
    organism_col = "organism_name", value_col = "antibiotic_value"
  ), "ggplot")
  expect_error(
    plot_resistance_by_agebin(d,
      mode = "single",
      organism_col = "organism_name", value_col = "antibiotic_value"
    ),
    "'center' must be provided"
...
test_that("plot_resistance_by_organism returns ggplot for all modes and errors correctly", {
  d <- make_amr_data()
  expect_s3_class(plot_resistance_by_organism(d,
    mode = "overall",
    organism_col = "organism_name", value_col = "antibiotic_value"
  ), "ggplot")
  expect_s3_class(plot_resistance_by_organism(d,
  ...
  bad$organism_name <- NULL
  expect_error(
    plot_resistance_by_organism(bad,
      mode = "overall",
      organism_col = "organism_name", value_col = "antibiotic_value"
    ),
    "Column\\(s\\) not found"
